...
CREATE TABLE `empleados` (
  ...
  `codDepto` varchar(4) COLLATE utf8_bin,
  PRIMARY KEY (`nDIEmp`),
  KEY `FK_Empl` (`jefeID`),
...
INSERT INTO `empleados` VALUES ('885.885','Raúl Duarte','M','1955-07-12','1998-05-16',1050000,200000,'Mecánico','333.333.333',NULL);
INSERT INTO `empleados` VALUES ('888.800','Marta Duro','F','1955-07-12','1998-05-16',1050000,200000,'Mecánico','333.333.333',NULL);
...
-- 1. Crear una vista top_salarios que te muestre los 5 empleados que más ganan.
create view top_salarios as
select empleados.* from empleados order by salEmp desc limit 5;

select * from top_salarios;

/*2. Crear vista que muestre los 10 empleados con más
antigüedad en la empresa, mostrando el nombre del
empleado y departamento y el año. Todos empleados
pertenezcan o no a algún departamento.*/
create view antiguedad as
select empleados.nomEmp, departamentos.nombreDpto,empleados.fecIncorporacion from empleados left join departamentos on empleados.codDepto = departamentos.codDepto
order by fecIncorporacion limit 10 ;

-- 3. Otorgar una comisión de 1000 a los empleados que pertenecen al departamento de PRODUCCIÓN.
update empleados set comisionE = comisionE + 1000 where codDepto = (select codDepto from departamentos where nombreDpto like'PRODUCCIÓN');

--  4. Borrar los empleados que no pertenezcan a ningún departamento
delete empleados from empleados  left join departamentos on empleados.codDepto = departamentos.codDepto where empleados.codDepto is null ;

-- 5. Crear una tabla salarios_deptos que muestre los salarios por departamento y sexo
create table salarios_deptos select departamentos.nombreDpto,empleados.sexEmp ,sum(empleados.salEmp) from empleados,departamentos where empleados.codDepto = departamentos.codDepto 
group by departamentos.nombreDpto,empleados.sexEmp order by nombreDpto;
