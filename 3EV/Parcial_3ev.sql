...
create table alumnosPorCurso select id_curso_escolar, count( id_alumno) from alumno_se_matricula_asignatura group by id_curso_escolar;

/*2. Modificar el nombre de las profesoras de tal manera que su nombre empiece por
Dra. Realizar en una transacción y realizar una cancelación de la transacción. Ej: Dra
María*/
set @@autocommit = 0;
start transaction;
update persona inner join profesor on persona.id = profesor.id_profesor and persona.sexo like 'M' set nombre = concat_ws(' ','dra',nombre);
rollback;

/*3. Borrar las personas nacidas en el año 2000. Si la consulta te da error activa:
SET FOREIGN_KEY_CHECKS = 0;
Realizar una transacción y cancelar el borrado.*/
SET FOREIGN_KEY_CHECKS = 0;
start transaction;
delete persona from persona where year(fecha_nacimiento) = 2000;
rollback;

/*4. Crear una vista que te muestre el nombre del alumno y en cuántos créditos se ha
matriculado*/
start transaction;
create view alumno_creditos as select persona.nombre, sum(asignatura.creditos) from persona,alumno_se_matricula_asignatura, asignatura 
where persona.id = alumno_se_matricula_asignatura.id_alumno and alumno_se_matricula_asignatura.id_asignatura = asignatura.id group by persona.id;
commit;

/*5. Crea una vista que te muestre los profesores de primero: Debe mostrar el id del
profesor, el nombre y el nombre de la asignatura.*/
start transaction;
create view profesores_primero as select profesor.id_profesor,persona.nombre, asignatura.nombre as asignatura from profesor,persona, asignatura 
where profesor.id_profesor= persona.id and profesor.id_profesor = asignatura.id_profesor and curso= 1;
commit;
select * from profesores_primero;

-- 6. Crea una vista que te muestre al personal docente (Usar multitabla)
start transaction;
create view personal_docente as select persona.* from persona, profesor where persona.id = profesor.id_profesor order by id;
commit;
